...
class TestBookRoutes(TestCase):
    ...
    def test_create_book_validation_error(self):
        ...
        payload = {"title": None, "author": 258, "quantity": 5, "penalty_fee": 2.5}
    ...
    def test_get_books(self):
        ...
        book1 = Book(title="Test Book", author="John Doe", quantity=4, penalty_fee=20)
        book2 = Book(title="Test Book", author="John Doe", quantity=4, penalty_fee=20)
        book3 = Book(title="Test Book", author="John Doe", quantity=4, penalty_fee=20)
        book4 = Book(title="Test Book", author="John Doe", quantity=4, penalty_fee=20)
    ...
    def test_get_by_title(self):
        """Check get_by_title endpoint return appropriate book data."""
        book1 = Book(title="Test Book", author="John Doe", quantity=4, penalty_fee=20)

        book2 = Book(
            title="Another book", author="John Doe", quantity=4, penalty_fee=20
        )

        book3 = Book(
            title="The Last Book", author="Mitchelle White", quantity=4, penalty_fee=20
        )
        ...
        response = self.client.get("/api/books/get_by_title/book")

        self.assertEqual(response.status_code, 200)
        self.assertEqual(len(response.json), 10)  # pagination object has 10 keys
        self.assertEqual(1, response.json["books"][0]["id"])
        self.assertEqual("Test Book", response.json["books"][0]["title"])
        self.assertEqual("John Doe", response.json["books"][0]["author"])
        self.assertEqual(4, response.json["books"][0]["quantity"])
        self.assertEqual(20, response.json["books"][0]["penalty_fee"])

    def test_get_by_title_on_empty_return(self):
        """Check if get_by_id endpoint returns an error message if book is not found."""
        response = self.client.get("/api/books/get_by_title/bob")
    ...
    def test_get_by_author(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(len(response.json), 10)  # pagination object has 10 keys
        self.assertEqual(3, response.json["books"][0]["id"])
        self.assertEqual("The Last Book", response.json["books"][0]["title"])
        self.assertEqual("Mitchelle White", response.json["books"][0]["author"])
        self.assertEqual(4, response.json["books"][0]["quantity"])
        self.assertEqual(20, response.json["books"][0]["penalty_fee"])

    def test_update(self):
        """Check if update route updates book."""
        book = Book(title="Test Book", author="John Doe", quantity=4, penalty_fee=20)
        ...
        response = self.client.put(
            f"/api/books/update/{retrieved_book.id}", json=update_data
        )
    ...
    def test_update_on_invalid_data(self):
        ...
        update_data = {"title": None, "author": 258, "quantity": 10, "penalty_fee": 15}
    ...
    def test_update_on_invalid_id(self):
        """Check if update route returns error on invalid id."""
        update_data = {
            "title": "Updated Book",
            "author": "Jane Doe",
            "quantity": 10,
        ...
        response = self.client.put("/api/books/update/100", json=update_data)
